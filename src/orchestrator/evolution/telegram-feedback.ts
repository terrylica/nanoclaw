...
function processCallback(
  ...
  switch (action) {
    ...
    case 'dismiss':
      answerCallback(query.id, 'Dismissed').catch((err: unknown) => {
        logger.warn({ err }, 'Failed to answer callback query');
      });
    ...
    case 'approve':
      ...
      answerCallback(query.id, 'Approved').catch((err: unknown) => {
        logger.warn({ err }, 'Failed to answer callback query');
      });
    ...
    case 'reject':
      ...
      answerCallback(query.id, 'Rejected').catch((err: unknown) => {
        logger.warn({ err }, 'Failed to answer callback query');
      });
    ...
    default:
      answerCallback(query.id, 'Unknown action').catch((err: unknown) => {
        logger.warn({ err }, 'Failed to answer callback query');
      });
  }
}
